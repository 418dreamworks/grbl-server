...
while current_z > target_z_limit:
    ...
    level_lines.append(f'G0 X{center_x + helix_radius:.3f}')
    ...
    while plunge_z > target_z:
        ...
        plunge_z -= descend
        level_lines.append(f'G3 X{center_x + helix_radius:.3f} Y{center_y:.3f} Z{plunge_z:.3f} I{-helix_radius:.3f} J0 F{feed:.0f}')
    ...
    if depth_so_far >= self.depth:
        target_radius = bore_radius
    else:
        target_radius = bore_radius - finish_stock
    ...
    while current_radius < target_radius:
        ...
        current_radius += step
        level_lines.append(f'G1 X{center_x + current_radius:.3f} F{feed:.0f}')
        level_lines.append(f'G3 X{center_x + current_radius:.3f} Y{center_y:.3f} I{-current_radius:.3f} J0 F{feed:.0f}')

    # Return to center for next level
    level_lines.append(f'G0 X{center_x:.3f}')
